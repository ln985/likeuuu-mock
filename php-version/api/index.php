 ...
 * 上传到虚拟主机即可运行，无需 Node.js
 * 
 * 文件结构:
 *   api/index.php    ← 本文件
 *   api/.htaccess    ← URL 重写规则
 *   api/data/        ← 数据文件目录
 */
...
// GET /app/config
if ($method === 'GET' && ($uri === 'app/config' || $uri === 'app/config/')) {
    jsonResponse(getAppConfig());
}
// GET /app/announcement/active
elseif ($method === 'GET' && ($uri === 'app/announcement/active' || $uri === 'app/announcement/active/')) {
    $limit = isset($query['limit']) ? intval($query['limit']) : null;
    jsonResponse(getActiveAnnouncements($limit));
}
// GET /location
elseif ($method === 'GET' && ($uri === 'location' || $uri === 'location/')) {
    $adcode = isset($query['adcode']) ? $query['adcode'] : null;
    jsonResponse(getLocation($adcode));
}
// GET /location/:adcode
elseif ($method === 'GET' && preg_match('#^location/(\d+)/?$#', $uri, $matches)) {
    jsonResponse(getLocation($matches[1]));
}
// GET /location/provinces
elseif ($method === 'GET' && ($uri === 'location/provinces' || $uri === 'location/provinces/')) {
    jsonResponse(getProvinces());
}
// GET /location/cities
elseif ($method === 'GET' && ($uri === 'location/cities' || $uri === 'location/cities/')) {
...
elseif ($method === 'GET' && ($uri === 'location/districts' || $uri === 'location/districts/')) {
    $cityAdcode = isset($query['cityAdcode']) ? $query['cityAdcode'] : null;
    jsonResponse(getDistricts($cityAdcode));
}
// GET /health
elseif ($method === 'GET' && ($uri === 'health' || $uri === 'health/')) {
    jsonResponse(['status' => 'ok', 'time' => date('c')]);
}
// POST /admin/announcement
elseif ($method === 'POST' && ($uri === 'admin/announcement' || $uri === 'admin/announcement/')) {
    $input = json_decode(file_get_contents('php://input'), true);
    jsonResponse(addAnnouncement($input));
}
// DELETE /admin/announcement/:id
elseif ($method === 'DELETE' && preg_match('#^admin/announcement/(\d+)/?$#', $uri, $matches)) {
    jsonResponse(deleteAnnouncement(intval($matches[1])));
}
// GET /admin/announcements
elseif ($method === 'GET' && ($uri === 'admin/announcements' || $uri === 'admin/announcements/')) {
